			...
			switch (id)
			{
				case "click_button":
                    ...
                    switch(Std.string(params[0]))
                    {
                        case "build":
                            {
                                Global.log("Building mech...");
                                Global.player.setMechanica(Std.parseInt(slotDrop.selectedId));
                                if(Global.player.getMechanica().buildFromParts(
                                ...
                                {
                                    Global.log("Build succeeded");
                                    Global.player.getMechanica().name = input.text;
                                    haxe.Timer.delay(backToMain, 5000);
                                   
                                }
                                else
                                {
                                    Global.log("Build failed");
                                }
                            }
                        case "back":
                            {
